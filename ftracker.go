...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	mendSpeedkmhInMsec := meanSpeed(action, duration) * kmhInMsec

	return ((walkingCaloriesWeightMultiplier*weight + ((mendSpeedkmhInMsec*mendSpeedkmhInMsec)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
